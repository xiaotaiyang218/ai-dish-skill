...
2. 归一化并理解菜品：
   ...
   - 图片输入要区分“菜名不确定”和“主要食材较确定”：如果看不准标准菜名，但能明确看到虾、蟹、蛋、动物内脏等高风险食材，应把食材事实用于过敏和健康约束判断，同时说明菜名仍需确认。
   - 推断可能食材、做法和风险标签，并区分已确认事实和合理假设。
3. 比对用户约束：
   - 过敏源和明确疾病限制优先级最高。
   - 低盐、控糖、减脂、高蛋白、素食、低脂等目标会影响推荐等级。
   - 推荐等级必须和证据一致：如果命中动物内脏、高嘌呤或多个高油/高盐/高糖/高脂标签，即使用户未声明限制，也不要直接给 `推荐`，应至少给 `谨慎` 并解释原因。
   - 证据不足、图片候选冲突或数据缺失时，返回 `需要确认`，不要强行给确定结论。
4. 用中文解释推荐：
   - 自然语言交流默认按“菜肴信息 -> 结论 -> 原因 -> 建议 -> 需要确认 -> 安全提示”的顺序回答。
   - 菜肴信息至少包括识别菜品、主要食材、常见做法、风险标签；如有标准份量数据，再给出热量、蛋白质、脂肪、碳水、糖和钠等精确营养估算；如果只有常见做法参考，只能输出“约 X-Y”的区间估算，并明确不是实测克重。
   - 结论只能是 `推荐`、`谨慎`、`不推荐` 或 `需要确认`，并给出 2-4 条与食材、做法、营养风险或用户约束直接相关的理由。
   - 只有在缺失信息会改变判断时，才向用户追问；不要只给结论而省略分析过程。
...
- 反馈闭环：运行 `scripts/apply_feedback.py`，处理 `accept`、`reject`、`favorite`、`correct_dish_name`、`set_user_profile` 等反馈事件。

## 执行与反馈状态

- 调用 `scripts/recommend.py` 代表本次已经执行确定性推荐引擎，并会记录一条 `recommendation_input` 分析输入事件，便于复核最近分析过什么。
- 记录分析输入不等于写入用户偏好反馈。只有用户明确说“接受这个建议”“不喜欢/不适合”“收藏”“实际菜名是 X”等反馈时，才调用 `scripts/apply_feedback.py` 写入 `accept`、`reject`、`favorite` 或 `correct_dish_name`。
- 当用户问“反馈给了吗”时，应明确回答：本次分析是否执行、分析输入是否已记录、偏好反馈是否已写入；如果尚未写入，应询问是否现在记录。
- `recommend.py` 输出中的 `execution_status` 表示脚本执行状态，`feedback_status` 表示分析输入与偏好反馈的记录状态。

## 用户画像记忆

- 只有用户明确确认“以后都记住我海鲜过敏”“最近在减脂/低盐”等信息时，才调用 `scripts/apply_feedback.py` 的 `action=user_profile` 写入画像；不要从一次普通提问里擅自长期记忆。
- `persistent_constraints` 用于长期约束，例如过敏、疾病相关限制、长期忌口；`temporary_goals` 用于阶段目标，例如最近减脂、控糖、低盐。
- 推荐请求带 `user_id` 时，`scripts/recommend.py` 会自动合并已确认的用户画像，并在结果中返回 `applied_user_profile`；自然语言回答应说明“已应用用户画像”，方便用户复核。
- 如果用户说某个阶段目标已结束，应更新或清理对应目标，不要把临时目标永久当作过敏或疾病限制。
...
- OpenClaw 核心用例手测清单：`references/openclaw-core-use-cases.md`
...
- `nutrition_quantitative_range`：无标准配方但有本地常见菜谱参考时可输出，必须标注为“常见范围估算，不是实测克重”。

自然语言回答应优先使用 `human_readable_cn` 风格；只有用户明确要求 JSON、脚本测试或上层程序调用时，才直接输出纯 JSON。
...
- 没有可靠数据源时，不编造精确热量、蛋白质、钠、糖或脂肪数值；如输出常见范围估算，必须用区间和“约”字表达。
