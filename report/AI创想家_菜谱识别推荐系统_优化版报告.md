...
当前能力分为三层：第一，**已实现能力**，包括本地菜谱库与营养知识库联动、规则化健康比对、中文可解释输出、命中标准配方时的营养量化，以及 `accept / reject / favorite / correct_dish_name` 最小反馈闭环；第二，**已建链路但保留降级的能力**，包括图片输入、OCR / vision provider、`raw_image_result` 中间结果保留、图片样本底表和多模态测试；第三，**待继续增强的能力**，包括更强的真实视觉融合、更广的标准配方量化覆盖和基于真实长期数据的反馈学习。

系统优先使用国内中式菜谱与营养数据源。当前运行时主依赖为本地结构化菜谱、营养知识库和标准量化配方；CookBook-KG 与 USDA FoodData Central 作为在线 fallback；百度菜品识别等能力作为可选增强；老乡鸡标准化菜谱、中国营养学会资料库、FoodWake、NutriData 等保留为后续扩展来源。项目围绕“数据、算力、算法”三要素构建方案：以结构化菜谱、食材营养表和用户健康约束作为数据基础，以 OCR、视觉识别和文本解析能力作为算力支撑，以多输入归一化、相似度匹配、规则推理和轻量反馈偏置作为算法核心。
...
**创新点三：用户反馈驱动的可解释推荐机制。**  
系统已实现 `accept`、`reject`、`favorite`、`correct_dish_name` 四类反馈事件的最小闭环。反馈数据会影响后续推荐中的纠错映射、confidence、解释文本以及部分推荐等级：例如 `favorite` 可提升置信度并在说明中体现偏好信号，`reject` 会降低推荐优先级并提示近期反馈偏好，`correct_dish_name` 则作为强纠错映射参与后续归一化。与只做一次性判断的规则系统相比，这使推荐结果具备“可被用户修正、可被后续调用利用”的持续优化能力；但当前实现仍是本地轻量闭环，不是训练式排序系统或强化学习系统。
...
| 在线 fallback | CookBook-KG、USDA FoodData Central | 本地缺失时补充候选菜谱或食材营养参考 | 已接入，但以降级方式使用 |
...
菜谱匹配流程为：输入菜名、菜单文字、OCR 文本或菜肴图片后，系统先进行标准菜名归一化，再从本地菜谱库召回候选菜谱，并在需要时接入 OCR / vision 候选、CookBook-KG fallback 和别名映射进行排序。营养匹配流程为：候选菜谱食材映射到标准食材名，再结合本地营养知识库与标准量化配方做健康判断；命中标准配方时输出定量营养结果，未命中时只输出风险标签、解释依据和人工确认提示。
...
**第二层：正式提交前仍需补充的真实统计指标。**

| 测试项目 | 测试内容 | 指标 | 当前状态 |
| --- | --- | --- | --- |
| OCR 识别 | 清晰菜单、模糊菜单、倾斜图片、手写备注 | 文字识别准确率、字符错误率 | 待继续实测统计 |
| 多模态识别 | 菜名 + 照片、菜单文字 + 照片 | Top-1 / Top-3 标准菜名识别率 | 待继续实测统计 |
| 候选菜谱匹配 | 常见菜、歧义菜名、地方菜名 | 候选召回率、人工复核通过率 | 待继续实测统计 |
| 健康比对 | 过敏、控糖、低盐、减脂场景 | 规则命中正确率 | 待继续实测统计 |
| 推荐解释 | 推荐、谨慎、不推荐样例 | 理由完整性、用户可理解性 | 待继续用户侧评估 |

这样处理的原因是：当前仓库已经具备自动化验证证据，但尚未形成适合直接写入比赛材料的大规模真实统计，因此报告应同时体现“已完成验证”和“仍需补实测”的边界。
...
5. 已实现 `accept / reject / favorite / correct_dish_name` 最小反馈闭环。
6. 已对一批标准配方菜品输出标准份量营养量化结果，并保留未命中标准配方时的边界说明。

下一步仍需继续补齐的方向包括：扩大量化菜谱覆盖、继续采集真实测试指标、提升复杂图片场景下的识别稳定性，以及在更长周期数据上验证反馈学习效果。
...
- **已实现（implemented）**：本地菜谱库 + 营养知识库联动的健康比对引擎；结构化 JSON 输出；中文可解释输出；`accept / reject / favorite / correct_dish_name` 最小反馈闭环；命中标准配方时的营养量化；自动化回归与报告对齐验证。
...
2. **联网能力状态化**：在 provider 层保留 `validated / needs_credentials / unavailable / degraded` 四态记录，并通过 `dish-health-recommender/references/provider-setup.md` 与相关脚本说明其配置和降级方式。
