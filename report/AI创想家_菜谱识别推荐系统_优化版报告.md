...
系统把本地结构化菜谱、营养知识库、标准量化配方和可选外部候选源组合起来，优先使用可复核的本地数据完成核心推荐，再把 CookBook-KG、中文菜谱站点、Spoonacular、百度菜品识别等外部来源作为补充候选。这样既能在网络条件差时保留可运行能力，又能在数据缺口处逐步扩展地方菜、标准配方和热量估算覆盖。量化输出只在命中标准配方或有足够食材估算依据时给出，例如提拉米苏约 390 kcal、草头圈子约 430 kcal，并同时保留计算来源和单位。
